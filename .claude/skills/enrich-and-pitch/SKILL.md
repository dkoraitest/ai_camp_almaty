...
#### Stage 4a — Identify Top Hooks (новое, обязательное)

**Цель:** до генерации сообщения — выделить **1-3 точки захода** по иерархии ценности.

Применяй tier-based ранжирование собранных данных:

| Tier | Что в нём | Hook-потенциал |
|---|---|---|
| **T1** | Strategic moments последних 6-12 мес: новый деал, интеграция, новый сегмент, **формирование новой функции** (= открыли первый senior hire в эту вертикаль) | Game-changing. Без T1-хука outbound в темноту |
| **T2** | Recent activity ЛПР (что лично говорил/делал) с конкретным фактом + Hiring formation moments (НЕ счётчик, а «впервые открыли X») | Critical для персонализации. Делает сообщение не-шаблонным |
| **T3** | Sector positioning, customer base, **gaps в найме** (где НЕТ людей → implicit needs) | Strong framing context |
| **T4** | Юр.данные, awards, размер, salary benchmarks | Supporting/credibility. Редко становятся hook'ом |
| **T5** | Адрес, телефон, generic descriptions | Шум |

**Логика выбора хуков:**
1. **Каждый hook = пересечение** их недавнего события / контекста × нашего оффера
2. Бери 1-3 хука максимум. Меньше — ок. Не накручивай T3/T4 если T1-T2 уже сильные
3. Hook-title: 5-8 слов, конкретно
4. Hook-body: 2-3 короткие фразы с **конкретными фактами + датами + именами**
5. Если winning T1 hook не нашли — честно скажи в Stage 5 («слабые сигналы для outreach, рекомендую generic company-level»)

**Хороший hook (Documentolog example):**
- T1: «Контур + partnership lead hiring» — февральская интеграция с Контур + только что открыли позицию руководителя международных партнёрских продаж. Формирование коммерческой функции с нуля.

**Плохой hook:**
- «Большая компания, лидер сегмента» — T3 без интерпретации, не winning
- «13 вакансий открыто» — T2 без интерпретации, просто счётчик
